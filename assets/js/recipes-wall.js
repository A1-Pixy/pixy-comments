  ...
  function init() {
    ...
    window.addEventListener("DOMContentLoaded", async () => {
      ...
      async function loadFeed(recipeId) {
        ...
        try {
          const r = await fetch(
            `/.netlify/functions/recipes-get?recipeId=${encodeURIComponent(recipeId)}&limit=50`
          );
      ...
      el.verifyCode?.addEventListener("click", async () => {
        ...
        if (!email || !token)
          return setStatus("Enter email and code.");
        setStatus("Verifying...");
        ...
        if (error) return setStatus(error.message);
        if (!data?.session)
          return setStatus("Sign-in failed.");
        setStatus("Signed in.");
        await refreshAuthUI();
      });
      ...
      el.submit?.addEventListener("click", async () => {
        ...
        if (kind === "recipe" && !title)
          return setStatus("Enter a title.");
